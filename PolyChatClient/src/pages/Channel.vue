...
export default {
  ...
  created() {
    ...
    this.socket.on("greetings", (arg) => {
      ...
      // Send greetings back
      this.socket.emit("greetings-back", `Wesh wesh cane a peche, c'est moi ${user}`);
    })
    ...
    this.socket.emit("get-channel", user);


    // The following calls are for test purposes, they need to be summoned by user input
    this.socket.emit("connect-channel", "1", user); // When clicking on a channel for example (could redirect to a new page or change the current one)
    this.socket.emit("send-message-channel", user, "1", "Bonjour a tous mes petits gauchistes"); // When pressing enter / send message button
  }
}
</script>
